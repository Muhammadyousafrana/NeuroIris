...
const irisData = {
    ...
    labels: [
        // Setosa labels (0-49)
        ...Array(50).fill(0),
        // Versicolor labels (50-99)
        ...Array(50).fill(1),
        // Virginica labels (100-149)
        ...Array(50).fill(2)
    ],
...
function getTrainTestSplit(testRatio = 0.2) {
    const shuffled = shuffleData(irisData.data, irisData.labels);
    const testSize = Math.floor(shuffled.data.length * testRatio);
    const trainSize = shuffled.data.length - testSize;
    
    return {
        trainData: shuffled.data.slice(0, trainSize),
        trainLabels: shuffled.labels.slice(0, trainSize),
        testData: shuffled.data.slice(trainSize),
        testLabels: shuffled.labels.slice(trainSize),
        classNames: irisData.classNames
    };
}
